...
public class Leaderboard : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        FirebaseDatabase.DefaultInstance
            .GetReference("users").OrderByChild("highscore").LimitToLast(5) // Limitar a los 5 mejores puntajes
    ...
    private void HandleValueChanged(object sender, ValueChangedEventArgs args)
    {
        ...
        var leaderboardData = snapshot.Children.Cast<DataSnapshot>()
            .OrderByDescending(x => Convert.ToInt32(x.Child("highscore").Value)).Take(5).ToList();
        ...
        foreach (var userDoc in leaderboardData)
        {
            ...
            go.GetComponent<ScoreEntry>().SetLabels("" + userObject["username"], "" + userObject["highscore"]);
